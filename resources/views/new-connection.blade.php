...
<nav class="glass-nav" id="glassNav">
    <a href="#" class="nav-logo-link">
        <img src="/images/logo.jpg" alt="SuperSpeed" class="nav-logo-img">
        <span class="nav-brand">SuperSpeed</span>
    </a>
...
<section class="page-content" style="padding: 160px 24px 100px; max-width: 800px; margin: 0 auto; color: white; text-align: center;">
    <h1 style="font-size: clamp(32px, 5vw, 48px); margin-bottom: 16px; font-family: 'Space Grotesk', sans-serif;">Request New Connection</h1>
    <p style="color: var(--text-secondary); margin-bottom: 48px; font-size: 17px;">Fill out the form below and our team will get in touch with you shortly.</p>
    ...
    <div class="form-container">
        ...
        <form action="{{ route('new-connection.store') }}" method="POST">
            @csrf
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="mobile" name="mobile" class="form-control" required placeholder="e.g. 017XXXXXXXX">
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select id="package_id" name="package_id" class="form-control" required style="appearance: none;">
                        <option value="">-- Choose a package --</option>
                        @foreach($packages as $pkg)
                            <option value="{{ $pkg->id }}">{{ $pkg->name }} ({{ $pkg->speed_mbps }} Mbps - ৳{{ $pkg->price }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                <label for="address">Full Installation Address *</label>
                <textarea id="address" name="address" class="form-control" rows="3" required placeholder="Division, District, Thana, Detailed Address"></textarea>
            </div>

            <div class="form-group">
                <label for="remarks">Additional Remarks</label>
                <textarea id="remarks" name="remarks" class="form-control" rows="2" placeholder="Any special instructions or questions?"></textarea>
            </div>

            <button type="submit" class="btn-pkg featured-btn" style="width: 100%; border: none; cursor: pointer; font-family: inherit;">Submit Request 🚀</button>
        </form>
    </div>
</section>
<!-- ═══ FOOTER ═══ -->
<footer>
    <div class="container">
        <div class="footer-grid">
            <div>
                <a href="#" class="footer-brand-logo"><img src="/images/logo.jpg" alt="SuperSpeed"><span>SuperSpeed Net</span></a>
                <p class="footer-brand-text">Bangladesh's premium dedicated fiber ISP. Fast, reliable and BTRC compliant since 2020.</p>
            </div>
            <div class="footer-col"><h4>Packages</h4><a href="#packages">Starter 5 Mbps</a><a href="#packages">Home 10 Mbps</a><a href="#packages">Power 20 Mbps</a><a href="#packages">Business 50 Mbps</a><a href="#packages">Enterprise 100 Mbps</a></div>
            <div class="footer-col"><h4>Company</h4><a href="#">About Us</a><a href="#">Coverage</a><a href="#contact">Contact</a><a href="#">Support</a></div>
            <div class="footer-col"><h4>Legal</h4><a href="https://www.btrc.gov.bd" target="_blank">BTRC Tariff Matrix</a><a href="#">Privacy Policy</a><a href="#">Terms of Service</a><a href="#">SLA Agreement</a></div>
        </div>
        <div class="footer-bar">
            <p>© 2024 SuperSpeed Net. All rights reserved.</p>
            <div class="btrc-stamp">✅ BTRC Licensed ISP</div>
        </div>
    </div>
</footer>
        ...
        menus.forEach((menu, index) => {
            if (menu.children && menu.children.length > 0) {
                ...
                <div class="nav-item-dropdown">
                    <a href="${menu.url || '#'}" class="nav-link" data-index="${index}">${menu.name} ▾</a>
                    <div class="nav-dropdown">
                        ${menu.children.map(child => `<a href="${child.url || '#'}">${child.name}</a>`).join('')}
                    </div>
                </div>`;
            } else {
                html += `<a href="${menu.url || '#'}" class="nav-link" data-index="${index}">${menu.name}</a>`;
            }
        });
        
        // Add Admin/Dashboard CTA
        html += `<a href="/dashboard" class="nav-cta">⚡ Portal</a>`;
        
        group.innerHTML = pillHTML + html;
        
        // Re-init pill logic for new elements
